...
export default function TeamCard(props: Props) {
  ...
  const facebookLink = team_member.socialLinks && team_member.socialLinks.facebook ? (
    <Link to={{ pathname: team_member.socialLinks.facebook }} target="_blanck">
      <img src={fbIcon} alt="fb_icon" />
    </Link>
  ) : null;


  const linkedinLink = team_member.socialLinks && team_member.socialLinks.linkedin ? (
    ...
    >
      <>
        <div className={[styles.card_avatar].join('')}>
          <img
            src={`${path}/public-api/team_member/${team_member.id}/photo` || avatarIcon}
            alt='avatar'
          />
        </div>
        <div className={[styles.card_info].join('')}>
          <div>
            <div className={[styles.card_info_header].join('')}>
              <h2>{team_member.firstName} {team_member.lastName}</h2>
